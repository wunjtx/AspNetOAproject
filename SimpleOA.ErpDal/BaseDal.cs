    ...
    public class BaseDal<T> where T: class,new()
    {
        ...
        public IQueryable<T> GetEntitiesIndex<S>(int pageIndex, int pageCount,out int Total, Expression <Func<T,bool>> whereLambda,Expression<Func<T,S>> orderbyLambda,bool isAsc)
        {
            ...
            if (isAsc)
            {
                ...
                .Skip(pageIndex * (pageCount - 1))
                .Take(pageIndex);
            }
            else
            {
                return dbContext.Set<T>().Where(whereLambda).OrderByDescending<T, S>(orderbyLambda).Skip<T>(pageIndex * (pageCount - 1)).Take(pageIndex);
            }
        }
